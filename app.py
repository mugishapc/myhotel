...
def dashboard():
    ...
    try:
        ...
        # Get today's income
        cursor.execute('SELECT SUM(price) as sum FROM sales WHERE date = %s', (today,))
        today_income_result = cursor.fetchone()['sum']
        today_income = today_income_result if today_income_result else 0
        
        # Get today's expenses
        cursor.execute('SELECT SUM(amount) as sum FROM expenses WHERE date = %s', (today,))
...
def reports():
    ...
    try:
        ...
        if session['role'] == 'admin':
            cursor.execute(
                'SELECT SUM(price) as sum FROM sales WHERE date BETWEEN %s AND %s',
                (start_date, end_date)
            )
            ...
            cursor.execute(
                'SELECT SUM(amount) as sum FROM expenses WHERE date BETWEEN %s AND %s',
                (start_date, end_date)
            )
            ...
            cursor.execute(
                'SELECT SUM(price) as sum FROM sales WHERE gestionnaire_id = %s AND date BETWEEN %s AND %s',
                (session['user_id'], start_date, end_date)
            )
            total_income_result = cursor.fetchone()['sum']
            total_income = total_income_result if total_income_result else 0
            ...
            cursor.execute(
                'SELECT SUM(amount) as sum FROM expenses WHERE gestionnaire_id = %s AND date BETWEEN %s AND %s',
                (session['user_id'], start_date, end_date)
            )
            total_expenses_result = cursor.fetchone()['sum']
            total_expenses = total_expenses_result if total_expenses_result else 0
...
def delete_user(user_id):
    ...
    try:
        ...
        cursor.execute('SELECT COUNT(*) as count FROM sales WHERE gestionnaire_id = %s', (user_id,))
        sales_count = cursor.fetchone()['count']
        cursor.execute('SELECT COUNT(*) as count FROM expenses WHERE gestionnaire_id = %s', (user_id,))
